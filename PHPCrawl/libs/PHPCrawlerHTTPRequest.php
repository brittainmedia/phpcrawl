...
class PHPCrawlerHTTPRequest
{
   ...
   *
   * @return A|PHPCrawlerDocumentInfo
   */
   ...
   */
  protected function readResponseContent($stream_to_file = false, &$error_code, &$error_string, &$document_received_completely): string
  { 
  ...
  protected function prepareHTTPRequestQuery($query): string
  {
    ...
    // Replace url-specific signs back
      $query = str_replace(array('%2F', '%3F'), array('/', '?'), $query);
      $query = str_replace(array('%3D', '%26'), array('=', '&'), $query);
