...
class PHPCrawlerEncodingUtils
{
  ...
  public static function decodeHtmlEntities($string): string
  {
    ...
    $string = preg_replace_callback('/&#([0-9]{1,4});/ i', static function($m) {return chr($m[1]);}, $string);
    
    // Numeric character reference replacement (HEX), like &#x2f; => "/"
    $string = preg_replace_callback('/&#x([0-9a-z]{2,4});/ i', static function($m) {return chr(hexdec($m[1]));}, $string);
